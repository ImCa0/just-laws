...
**批量收录模式**：本技能支持批量收录模式，自动从 LAWS_PROGRESS.md 中查找未收录的法律，按照顺序依次处理每部法律。
...
调用本 skill 时需要提供以下参数：
- Markdown 文件路径（如 ".temp/laws_md/中华人民共和国爱国主义教育法_20231024.md"）

**说明**：
- **法律原文位置**：在 `.temp/laws_md/` 目录下查找，文件名包含待收录的法律名称
- 法律全名：从文件名或 md 内容中自动提取
- 英文名称：由 AI 自动翻译
- **法律分类**：直接从 LAWS_PROGRESS.md 中获取（每部法律所在的分类即为该法律的分类）

### 批量收录模式（推荐）

无需提供参数，技能将自动执行以下流程：
1. 读取 LAWS_PROGRESS.md，找到所有未收录的法律
2. 按照法律在文件中的顺序，依次处理每部未收录的法律
3. **在 `.temp/laws_md/` 目录下查找对应的法律原文 md 文件**（文件名包含法律名称）
4. 对每部法律执行完整的收录流程（格式化、创建文件、更新配置、提交代码）
5. 每收录一部法律后，立即更新 LAWS_PROGRESS.md 中的状态和进度
6. 为每部法律创建独立的 git commit
...
**确定法律分类**：
- 直接从 LAWS_PROGRESS.md 中查找该法律所在的分类
- 该法律在哪个分类下，就是该法律的分类
- 无需额外查询
...
**读取源文件**：
- **文件位置**：在 `.temp/laws_md/` 目录下查找，文件名包含待收录的法律名称
- 使用 Glob 或 Read 工具读取对应的 md 文件
- 保存原始内容用于参考
...
2. **更新分类进度**：
   ...
   - 更新分子数字（已收录数量）
   - 更新百分比
   - 示例：`**进度：14/88 (15.9%)**` → `**进度：15/88 (17.0%)**`

3. **更新总计进度**：
   - 更新文件顶部的总进度
   - 更新底部的总计统计
   - 示例：`**收录进度：50/308 (16.2%)**` → `**收录进度：51/308 (16.6%)**`

4. **更新统计表格**：
   - 在文件末尾的统计部分更新对应分类的进度
   - 示例：`- **经济法**：14/88 (15.9%)` → `15/88 (17.0%)`
...
- 根目录下的 `LAWS_PROGRESS.md`

**提交命令**：
```bash
git add docs/{分类}/{英文名称}/README.md docs/.vuepress/config.js LAWS_PROGRESS.md
...
3. 查询分类：从 LAWS_PROGRESS.md 查询 → 社会法
4. 检查是否存在：不存在
5. 读取 md 文件并格式化：
   - 提取立法修法记录
   - 格式化章节标题和条号
...
2. 翻译英文名称：`civil-code`
3. 查询分类：从 LAWS_PROGRESS.md 查询 → 民商法
4. 检查是否存在：不存在
5. 读取 md 文件并格式化：
   - 统计条文：1260条（超过200条）
   - 告知用户需要拆分文件
...
10. 更新 LAWS_PROGRESS.md：更新法律状态和进度统计
...
执行结果：
1. 读取 LAWS_PROGRESS.md，识别未收录的法律（共289部）
2. 按顺序处理第一部未收录的法律（例如：保守国家秘密法）：
   - 读取 `.temp/laws_md/中华人民共和国保守国家秘密法_20240227.md`
   - 格式化内容并创建文件
   - 更新 config.js
   - 更新 LAWS_PROGRESS.md
   - 提交：`git commit -m "📘 收录《保守国家秘密法》"`
3. 处理第二部未收录的法律（例如：公共图书馆法）：
   - 读取源文件
   - 格式化内容并创建文件
   - 更新 config.js
   - 更新 LAWS_PROGRESS.md
   - 提交：`git commit -m "📘 收录《公共图书馆法》"`
4. 依次处理剩余287部未收录的法律
5. 全部处理完成后，LAWS_PROGRESS.md 显示：`**收录进度：308/308 (100.0%)**`
...
- ✅ LAWS_PROGRESS.md 已更新（法律状态、分类进度、总计进度）
- ✅ 无重复条目
- ✅ 代码已提交（包含法律文件、config.js、LAWS_PROGRESS.md）

**批量模式额外标志**：
- ✅ 所有未收录法律均已处理
- ✅ LAWS_PROGRESS.md 显示全部收录：`**收录进度：308/308 (100.0%)**`
